...
# Disability: Already binary (Yes/No)
# Sex: Already binary (Male/Female)

# Check simplified distributions
cat("\nSimplified Race:\n")
print(table(gee_data_clean$race_simple))
cat("\nSimplified School:\n")
print(table(gee_data_clean$school_simple))
cat("\nSimplified Urban:\n")
print(table(gee_data_clean$urban_simple))

# Try model with simplified variables
model_math_main <- geeglm(
  math ~ time + I(time^2) +  
...
cat("\n", rep("=", 80), "\n", sep="")
cat("COMPREHENSIVE GEE MODEL DIAGNOSTICS\n")
cat(rep("=", 80), "\n\n", sep="")

# ----------------------------------------------------------------------------
# 1. MODEL SUMMARY AND FIT STATISTICS
# ----------------------------------------------------------------------------

# Sample size information
cat("Sample Size:\n")
cat("  Total observations:", nrow(model_math_main$data), "\n")
cat("  Number of clusters (children):", length(unique(model_math_main$id)), "\n")
cat("  Average obs per child:", 
    round(nrow(model_math_main$data) / length(unique(model_math_main$id)), 2), "\n")
cat("  Max cluster size:", max(table(model_math_main$id)), "\n\n")

# Working correlation structure
cat("Working Correlation Structure:", model_math_main$corstr, "\n")
if (model_math_main$corstr != "independence") {
  cat("Estimated correlation parameter(s):\n")
  print(model_math_main$geese$alpha)
}
...
diag_data <- data.table(
  ...
  time = gee_data_clean[!is.na(math) & !is.na(fs_scale) & !is.na(time) & 
                        !is.na(ses_baseline) & !is.na(sex) & !is.na(fs_baseline) &
                        !is.na(race_simple) & !is.na(disability) & !is.na(household_size) &
                        !is.na(urban_simple)]$time
)

# Residual summary statistics
cat("Residual Summary Statistics:\n")
cat("  Mean:", round(mean(residuals, na.rm = TRUE), 4), 
    " (should be ≈ 0)\n")
cat("  Std Dev:", round(sd(residuals, na.rm = TRUE), 3), "\n")
cat("  Min:", round(min(residuals, na.rm = TRUE), 3), "\n")
cat("  Max:", round(max(residuals, na.rm = TRUE), 3), "\n")
cat("  IQR:", round(IQR(residuals, na.rm = TRUE), 3), "\n\n")

# Check for outliers (|residual| > 3)
outliers <- sum(abs(residuals) > 3, na.rm = TRUE)
cat("Observations with |residual| > 3:", outliers, 
    "(", round(100 * outliers / length(residuals), 2), "%)\n\n")
...
legend("topright", c("Zero line", "±2 SD", "Lowess"), 
       col = c("red", "blue", "darkgreen"), 
       lty = c(2, 2, 1), lwd = 2, cex = 0.7)
...
legend("topright", "Normal curve", col = "red", lty = 1, lwd = 2)

# Plot 6: Residuals vs Predicted (by child - check within-cluster)
# Sample some children for clarity
set.seed(653)
...
plot(plot_data$time, plot_data$residuals, type = "n",
     ...
     main = "Within-Child Residual Patterns (30 children)")
